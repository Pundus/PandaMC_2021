...
execute as @s[scores={Pundus_MenuVar=301}] at @s run data modify entity @e[type=#pundus_root:frames,sort=nearest,distance=..3,limit=1,tag=!IFE_ForceLock] Fixed set value 0b
...
execute as @s[scores={Pundus_MenuVar=203}] at @s run tellraw @a[team=Admin,tag=AdminAlerts] {"translate":"pundus.admin.alerts.ife","color":"gray","italic":true,"with":[{"selector":"@s"},{"color":"white","translate":"pundus.ife.options.invis"},{"color":"white","translate":"pundus.ife.false"}]}
