...
execute if entity hugs_and_stabs as hugs_and_stabs at @s[gamemode=survival] if entity @e[type=minecraft:area_effect_cloud,tag=XalHome,limit=1,distance=..40] run tellraw Lord_Pundus {"text":"hugs in range"}
...
execute if entity hugs_and_stabs as hugs_and_stabs at @s[gamemode=adventure] unless block ~ 319 ~ minecraft:structure_void unless entity @e[type=minecraft:area_effect_cloud,tag=XalHome,limit=1,distance=..40] run tellraw Lord_Pundus {"text":"hugs out of range"}
...
tag @a[tag=Pundus_OH_Custom_Consumable,predicate=!pundus_root:generic/offhand_consumable_custom_item] remove Pundus_OH_Custom_Consumable
